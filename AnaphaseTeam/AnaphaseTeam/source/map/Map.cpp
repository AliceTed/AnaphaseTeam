...
Map::Map(OCTREE_ID _id, const GSvector3&_translate)
	:m_ID(_id),
...
{
	Data::CastID cast;
	return gsOctreeCollisionRay(gsGetOctree(cast(m_ID)),&(_position+m_translate),&_direction,_intersect, NULL)==GS_TRUE;
}

const bool Map::isCollisionSphere(const GSvector3 & _center, float _radius, GSvector3* _out_center) const
{
	Data::CastID cast;
	return gsOctreeCollisionSphere(gsGetOctree(cast(m_ID)), &(_center + m_translate),_radius,_out_center) == GS_TRUE;
}
